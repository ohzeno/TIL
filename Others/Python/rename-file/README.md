...
os.rename으로 파일/폴더의 이름을 변경할 수 있다.

----------------------------

백준에서 푼 문제들은 bj_12345_deque 이런 식으로 저장해뒀었다. 폴더를 통해 분류는 되지만 같은 분류의 문제끼리 구분이 힘들었다. 폴더 명과 파일 명을 문제의 이름으로 수정하려 했고, 세 자리 수 단위의 폴더와 파일을 손으로 수정할 수는 없어 파일을 작성했다.
